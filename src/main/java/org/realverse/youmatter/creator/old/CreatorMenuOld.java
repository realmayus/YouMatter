...
public class CreatorMenuOld extends AbstractContainerMenu {

    public CreatorBlockEntityOld creator;
    private IItemHandler playerInventory;


    public CreatorMenuOld(int windowId, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModContent.CREATOR_MENU_OLD.get(), windowId);
        creator = level.getBlockEntity(pos) instanceof CreatorBlockEntityOld creator ? creator : null;
    ...
    public boolean stillValid(Player player) {
        ...
        return !level.getBlockState(pos).is(ModContent.CREATOR_BLOCK_OLD.get()) ? false : player.distanceToSqr(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D) <= 64.0D;
    }
